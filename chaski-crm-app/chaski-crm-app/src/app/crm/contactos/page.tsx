    ...
    <div>
      ...
      <form className="flex flex-wrap gap-2 mb-4" method="get">
        <input
          ...
          className="border rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-brand-300 flex-1 min-w-[200px] bg-white"
        />
        <select
          name="etapa"
          defaultValue={etapa || ""}
        ...
        <button type="submit" className="border rounded-lg px-4 py-2 text-sm font-medium hover:bg-gray-100 bg-white">
          Filtrar
        </button>
      </form>
            ...
            {contactos.map((c: any) => {
              ...
              return (
                <tr key={c.id} className="border-t hover:bg-gray-50">
                  <td className="p-3">
                    <Link href={`/crm/contactos/${c.id}`} className="flex items-center gap-2.5 group">
                      <Avatar nombre={c.nombre} size="sm" />
                      <span>
                        <span className="font-medium text-ink group-hover:text-brand-600 block">{c.nombre}</span>
                        {c.email && <span className="text-xs text-gray-400 block">{c.email}</span>}
                      </span>
                    </Link>
                  </td>
